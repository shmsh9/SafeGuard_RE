...
void encryption_3(const uint16_t *buffer_base, uint16_t *output) {
...
void hash(uint8_t *input, uint16_t input_l, uint8_t *output, uint16_t *bx) {
    int outer_loops = 8;
    int i;
...
void encryption_1(const uint8_t hash[16], uint8_t output[104]) {
...
void encryption_2(uint8_t* SI, uint8_t* DI) {
    int BP = 8;
...
int main(int argc, char **argv){
	...
	for(int i = 0; i < sizeof(out); i++){
		hash(buff+i, 0x10, out, &hash_seed);
	}
	PHEX(out);
	...
	for(int i = 0; i < sizeof(out); i++){
		hash(in2+i, sizeof(in2), out, &hash_seed);
	}
	PHEX(out);
	uint8_t rebuff[0x34*sizeof(uint16_t)]= {0};
	encryption_1(out, rebuff);
	PHEX(rebuff);
	...
	encryption_2(in3, rebuff);
	encryption_2(in3+8, rebuff);
	PHEX(in3);

	uint8_t rebuff2[0x34*sizeof(uint16_t)] = {0};
	encryption_1(in3, rebuff2);
	PHEX(rebuff2);

	uint8_t out3[0x34*sizeof(uint16_t)] = {0};
	encryption_3((uint16_t*)rebuff2, (uint16_t*)out3);
	PHEX(out3);
